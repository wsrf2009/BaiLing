...
- (void)setup {
    ...
    self.initialSpringVelocity = /*9.0*/1.0;
    self.springDamping = /*0.8*/0.9;
}
...
- (void)presentationWithSide:(JVFloatingDrawerSide)drawerSide sideView:(UIView *)sideView centerView:(UIView *)centerView animated:(BOOL)animated completion:(void (^)(BOOL))completion {
    ...
    if (animated) {
        if ([SystemToolClass systemVersionIsNotLessThan:@"7.0"]) {
            [UIView animateWithDuration:self.animationDuration delay:self.animationDelay usingSpringWithDamping:self.springDamping initialSpringVelocity:self.initialSpringVelocity options:UIViewAnimationOptionCurveLinear animations:springAnimation completion:nil];
        } else {

            [UIView animateWithDuration:self.animationDuration animations:springAnimation];
        }
